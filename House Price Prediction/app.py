...
if predict_btn:
    ...
    loc_val = 2 if location == "Downtown / Prime" else (1 if location == "Suburbs" else 0)
    garage_val = 1 if garage == "Yes" else 0

    # Combine inputs into standard array shape [1, n_features]
    input_features = np.array([[sqft, bedrooms, bathrooms, loc_val, age, garage_val]])

    if model is not None:
        raw_price = model.predict(input_features)[0]
    else:
        # Fallback dummy logic if house_model.pkl is missing
        base_price = sqft * 150
    ...
    # Apply Selected Currency Conversion
